		...
		while (1) {
			...
			if (enable_mode_conn == TRUE) {
				if (current_mode == PREFIX_COMMENT_MODE) {
					expect_next_char(buffer[buffer_i], prev_mode, ret);
				} else {
			...
			for (; buffer[buffer_i] != '\0' && buffer[buffer_i] != EOF; buffer_i ++) {
				...
				// current_char = buffer[buffer_i];

				update_comment_mode(buffer);
				if (comment_mode != NONE_MODE) {
					printf("In the (%d, %d): mode = %d: comment_mode = %d: char = %c\n",
						row,
						column,
						current_mode,
						comment_mode,
					...
					if (current_mode == PREFIX_COMMENT_MODE) {
						...
						expect_next_char(next_char, prev_mode, ret);
					}
					else {
